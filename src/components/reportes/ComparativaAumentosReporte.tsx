...
const ComparativaAumentosReporte: React.FC = () => {
  ...
  const handleGenerarReporte = async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        setError(
          "No se encontraron variaciones de tarifas para el período seleccionado."
        );
      } else {
        setReporte(data);
      }
            ...
            <Paper key={item.tarifaId} sx={{ p: 2, my: 1 }}>
              ...
              <ListItemText
                ...
                secondary={`${formatCurrency(
                  item.valorInicial
                )} (al ${formatDate(item.fechaInicial)})`}
              />
              <ListItemText
                primary="Valor Final"
                secondary={`${formatCurrency(item.valorFinal)} (al ${formatDate(
                  item.fechaFinal
                )})`}
              />
              <ListItemText
            ...
            <TableBody>
              {reporte.map((item) => (
                <TableRow key={item.tarifaId}>
                  ...
                  <TableCell align="right">
                    {formatCurrency(item.variacionAbsoluta)}
                  </TableCell>
                  <TableCell align="right">
                    {formatPercentage(item.variacionPorcentual)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
